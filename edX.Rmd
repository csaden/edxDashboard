...
detailsBase <- read.csv("detailsBase.csv", header=T)
...
detailsProgress <- merge(detailsBase, scoresByLecture, all.x=T)
detailsProgress <- merge(detailsProgress, videosByLecture, all.x=T)
detailsProgress[is.na(detailsProgress)] <- 0

write.csv(detailsProgress, "details_progress.csv")
```
